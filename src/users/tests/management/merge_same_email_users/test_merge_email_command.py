...
def test_command_doesnt_handle_disabled_accounts(mixer, handle_single_email_mock):
    mixer.blend('users.User', email='alice@example.com', is_active=False)

    management.call_command('merge_same_email_users')

    handle_single_email_mock.assert_not_called()


def test_command_doesnt_handle_staff_accounts(mixer, handle_single_email_mock):
    mixer.blend('users.User', email='alice@example.com', is_staff=True)
