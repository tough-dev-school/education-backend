...
@pytest.mark.parametrize("paid_orders_quantity", [1, 5])
def test_should_be_applied_if_has_paid_order(tag_mechanism, factory, user, paid_orders_quantity):
    factory.cycle(paid_orders_quantity).order(user=user, paid=timezone.now())
...
def test_return_list_tags_for_paid_courses_and_groups(tag_mechanism, user, course, another_course_same_group):
...
@pytest.mark.parametrize("paid_orders_quantity", [1, 5, 10])
def test_nplusone_for_paid_courses_and_groups(tag_mechanism, user, django_assert_num_queries, paid_orders_quantity, factory):
    factory.cycle(paid_orders_quantity).order(user=user, paid=timezone.now())

    with django_assert_num_queries(2):
        got = tag_mechanism(user)()

    assert len(got) == 2 * paid_orders_quantity
