...
def test_existing_user(user) -> None:
...
def test_two_users_with_same_email(user, mixer) -> None:
...
def test_two_users_with_same_email_case_is_case_insensitive(user, mixer) -> None:
...
def test_existing_user_name_does_not_change(user) -> None:
    created = UserCreator(name="Камаз Отходов", email="rulon.oboev@gmail.com")()
...
def test_user_with_another_email_case_is_the_same(user) -> None:
    created = UserCreator(name="Камаз Отходов", email="RuLoN.oBoEV@gmAil.cOm")()
    assert created == user


def test_user_creates_if_existing_inactive(user) -> None:
    user.update(
        is_active=False,
