...
def test_no_crosscheckes(api, course, lesson, crosscheck):
    crosscheck.delete()
...
def test_not_checked(api, course):
    got = api.get(f"/api/v2/lessons/?course={course.slug}")
...
def test_checked(api, course, crosscheck):
...
def test_three_crosschecks(api, course, question, mixer):
...
def test_non_user_crosschecks_are_ignored(api, course, crosscheck, another_user):
...
def test_non_lesson_crosschecks_are_ignored(api, course, crosscheck, another_question):
...
def test_crosscheckes_where_current_user_is_author_are_ignored(api, course, crosscheck, answers, another_user):
