...
def test_return_pipeline_id(b2c_pipeline) -> None:
    got = b2c_pipeline_id()

    assert got == b2c_pipeline.id


def test_return_pipeline_status_id(unsorted_status) -> None:
    got = b2c_pipeline_status_id(status_name="unsorted")

    assert got == unsorted_status.id


def test_fail_if_no_such_pipeline(mock_get_pipelines) -> None:
...
def test_fail_if_no_such_status() -> None:
    with pytest.raises(AmoCRMCacheException):
