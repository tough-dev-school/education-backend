...
def test_sent(chain_sender, send_message, parent_message, study) -> None:
    chain_sender()

    send_message.assert_called_once_with(parent_message, study=study)


def test_root_message_is_only_sent_when_there_is_no_progress_record(chain_sender, send_message, parent_message, study, mixer) -> None:
...
def test_progress_records_from_another_studies_are_ignored(chain_sender, send_message, parent_message, study, another_study, mixer) -> None:
...
def test_two_root_messages_are_sent(chain_sender, send_message, parent_message, second_parent_message, study, mocker) -> None:
    chain_sender()
