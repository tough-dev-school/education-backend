...
def test_sent(send_message, parent_message, study) -> None:
    tasks.send_active_chains()

    send_message.assert_called_once_with(parent_message, study=study)


def test_sending_inactive_chains_are_not_sent(send_message, chain) -> None:
...
def test_archived_chains_are_not_sent(send_message, chain) -> None:
