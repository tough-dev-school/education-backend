...
def test_user_without_purchases_has_no_configuration(user) -> None:
    assert get_configuration(recipient=user.email) is None


@pytest.mark.parametrize("is_order_paid", [True, False])
def test_course_with_configuration(factory, configuration, user, is_order_paid) -> None:
...
@pytest.mark.parametrize("is_order_paid", [True, False])
def test_last_course_contains_no_configuration(factory, course, user, is_order_paid) -> None:
    factory.order(user=user, item=course, is_paid=is_order_paid)

    assert get_configuration(recipient=user.email) is None


def test_orders_without_itema_do_not_break_things(factory, configuration, user) -> None:
...
def test_courses_are_ordered_by_creation_1(factory, configuration, another_configuration, user) -> None:
...
def test_courses_are_ordered_by_creation_2(factory, configuration, another_configuration, user) -> None:
    order1 = factory.order(user=user, item=another_configuration.course)
    order2 = factory.order(user=user, item=configuration.course)
