...
def _enable_outgoing_email(settings) -> None:
...
def test_emaiL_is_sent(user, campaign) -> None:
    campaign.execute(user)

    assert len(mail.outbox) == 1


def test_email_is_sent_even_if_the_letter_was_sent_already(user, campaign) -> None:
...
def test_email_is_sent_to_the_right_place_with_the_right_template_id(user, campaign, send_mail) -> None:
    campaign.execute(user)
...
def test_email_is_sent_with_the_right_context(user, campaign, send_mail) -> None:
    campaign.execute(user)
