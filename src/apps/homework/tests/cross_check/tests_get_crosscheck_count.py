...
def test_no_crosschecks(dispatcher, answers) -> None:
    dispatcher = dispatcher(answers=answers)

    assert get_crosscheck_count(answers[0], dispatcher) == 0


def test_no_crosschecks_from_non_dispatched_users(dispatcher, mixer, answers) -> None:
...
def test_crosschecks(dispatcher, mixer, answers) -> None:
