...
def test_personal_root_answers_are_included(answer_one, answer_two, user) -> None:
    assert answer_one in answers(user)
    assert answer_two in answers(user)


def test_other_authors_are_excluded(answer_one, another_user, user) -> None:
    answer_one.update(author=another_user)

    assert answer_one not in answers(user)


def test_other_answers_included_for_user_with_see_all_answers_permissions(answer_one, user, another_user) -> None:
    answer_one.update(author=another_user)
...
def test_other_authors_are_included_if_seen(answer_one, another_user, user) -> None:
    answer_one.update(author=another_user)
...
@pytest.mark.usefixtures("answer_access_log_entry")
def test_no_weird_orm_quirks_on_answers_accessed_by_multiple_users(mixer, answer_one, another_user, user) -> None:
...
def test_child_answers_from_another_users_are_included(answer_one, answer_two, another_user, user) -> None:
...
def test_sibling_answers_are_included(answer_one, answer_two, root_answer, user, another_user) -> None:
...
def test_children_of_sibling_answers_are_included(answer_one, answer_two, root_answer, user, another_user, child_of_answer_two) -> None:
    answer_one.update(parent=root_answer)
    answer_two.update(author=another_user, parent=root_answer)
