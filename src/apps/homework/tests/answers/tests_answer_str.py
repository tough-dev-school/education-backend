...
def test_limited_to_30(mixer) -> None:
    answer = mixer.blend(
        "homework.Answer",
...
def test_starts_with_link(mixer, long_word, expected_string) -> None:
    answer = mixer.blend("homework.Answer", text=long_word)

    assert str(answer) == expected_string


def test_html(mixer) -> None:
