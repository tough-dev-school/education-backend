...
def test_answer_query_param_is_required(get_comments) -> None:
    got = get_comments(expected_status_code=400)

    assert "answer" in got


def test_fields(get_comments, answer) -> None:
...
def test_return_results_for_several_comma_separated_answers(get_comments, answer, another_answer) -> None:
    got = get_comments(f"?answer={answer.slug},{another_answer.slug}")

    assert len(got) == 2


def test_return_comments_for_root_answers_only(get_comments, answer_with_descendants) -> None:
...
def test_return_empty_result_for_non_existed_answers(get_comments) -> None:
    got = get_comments(f"?answer={uuid4()}")

    assert got == []


def test_exclude_not_allowed_to_access_answers(get_comments, answer_another_user) -> None:
    got = get_comments(f"?answer={answer_another_user.slug}")  # answer from another author and not previously accessed

    assert got == []


def test_include_previously_accessed_answers_from_other_authors(api, get_comments, answer_another_user, mixer) -> None:
...
def test_include_other_authors_user_with_see_all_answers_permissions(get_comments, answer_another_user, api) -> None:
