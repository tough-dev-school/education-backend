...
def test_changing_text(api, answer) -> None:
...
def test_patch_changing_text_response_fields(api, answer, another_answer) -> None:
...
def test_update_answer_without_parent_do_not_have_parent_field_in_response(api, answer) -> None:
...
def test_405_for_put(api, answer) -> None:
    api.put(f"/api/v2/homework/answers/{answer.slug}/", {"text": "*patched*"}, expected_status_code=405)


def test_only_answers_not_longer_than_a_day_may_be_edited(api, answer, freezer) -> None:
...
def test_answers_created_within_a_day_may_be_updated(api, answer, freezer) -> None:
...
def test_only_answers_without_descendants_may_be_edited(api, answer) -> None:
...
def test_403_without_a_purchase(api, answer) -> None:
    api.patch(f"/api/v2/homework/answers/{answer.slug}/", {"text": "*patched*"}, expected_status_code=403)


def test_404_for_answer_of_another_author(api, answer_of_another_author) -> None:
    api.patch(f"/api/v2/homework/answers/{answer_of_another_author.slug}/", {"text": "*patched*"}, expected_status_code=404)


def test_changing_text_response(api, answer) -> None:
    got = api.patch(f"/api/v2/homework/answers/{answer.slug}/", {"text": "*patched*"})
