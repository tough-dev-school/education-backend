...
def test_log_entry_is_created(api, answer) -> None:
...
def test_terrible_things_does_not_happen_when_there_already_is_a_log_entry_created(api, answer) -> None:
    for _ in range(2):
        api.get(f"/api/v2/homework/answers/{answer.slug}/", expected_status_code=200)


def test_log_entry_is_not_created_when_requesting_own_answers(api, answer) -> None:
...
def test_log_entry_is_not_created_for_users_with_permission(api, answer) -> None:
...
def test_log_entry_is_not_created_for_superusers(api, answer) -> None:
