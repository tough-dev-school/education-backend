...
def test_destroy_non_root_answer(api, answer, answer_of_another_author) -> None:
...
def test_only_answers_without_descendants_may_be_destroyed(api, answer) -> None:
...
def test_only_answers_not_longer_than_a_day_may_be_destroyed(api, answer, freezer) -> None:
    freezer.move_to("2032-12-02 15:30")

    api.delete(f"/api/v2/homework/answers/{answer.slug}/", expected_status_code=403)


def test_answers_created_within_a_day_may_be_destroyed(api, answer, freezer) -> None:
...
def test_can_not_destroy_answer_of_another_author(api, answer_of_another_author) -> None:
