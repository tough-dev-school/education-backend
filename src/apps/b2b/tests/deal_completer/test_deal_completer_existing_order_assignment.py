...
def test_shipped_but_not_paid_orders_are_not_ignored_during_order_asignment(completer, factory):
    deal = factory.deal(student_count=1)
    existing_order = factory.order(user=deal.students.first().user, item=deal.course)
    ...
    assert existing_order.deal == deal, "Deal has been changed, hence the order is not ignored"
