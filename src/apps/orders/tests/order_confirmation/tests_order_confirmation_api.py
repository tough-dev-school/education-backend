...
def test_respose(anon, order) -> None:
...
def test_response_on_already_confirmed_order(anon, confirmed_order) -> None:
...
def test_unconfirmed_order_ships(anon, order, ship) -> None:
...
def test_order_is_shipped_only_one_time(anon, order, ship) -> None:
    for _ in range(3):
        anon.get(f"/api/v2/orders/{order.slug}/confirm/", expected_status_code=302)
...
def test_404_for_non_zero_priced_orders(anon, order, ship) -> None:
