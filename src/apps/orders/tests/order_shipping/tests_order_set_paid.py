...
def test_ships(order, course, user, ship) -> None:
    order.set_paid()

    ship.assert_called_once_with(course, to=user, order=order)


def test_not_ships_if_order_is_already_paid(order, ship) -> None:
...
def test_shipment_date(order) -> None:
    order.set_paid()
    order.refresh_from_db()
...
def test_order_is_not_shipped_again_if_already_shipped(order, ship) -> None:
...
def test_shipment_date_is_not_reset(order) -> None:
...
def test_unpaid_date_is_reset(order) -> None:
...
def test_unpaid_date_is_not_reset_when_order_is_not_paid(order) -> None:
    order.update(
...
def test_empty_item_does_not_break_things(order, ship) -> None:
