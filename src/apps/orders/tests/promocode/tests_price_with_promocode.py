...
def test_promocode_with_bank(api, course, bank, expected_price, expected_formatted_price, expected_currency, expected_currency_symbol) -> None:
...
def test_bad_promocode(api, course, code) -> None:
    got = api.get(f"/api/v2/courses/{course.slug}/promocode/?promocode={code}")

    assert got["price"] == 100500


def test_incompatible_promocode(api, course, another_course, ten_percent_promocode) -> None:
...
def test_compatible_promocode(api, course, ten_percent_promocode) -> None:
    ten_percent_promocode.courses.add(course)
...
def test_wihtout_promocode(api, course) -> None:
    got = api.get(
