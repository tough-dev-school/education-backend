...
def test_call_update_cache(mock_fetch_page, mock_cache_set, material, page):
    update_cache(material.page_id)

    mock_cache_set.assert_called_once_with(material.page_id, page)
    mock_fetch_page.assert_called_once_with(material.page_id)


def test_call_update_cache_for_all_pages(mock_fetch_page, mock_cache_set, material, page, mixer):
    for _ in range(3):
