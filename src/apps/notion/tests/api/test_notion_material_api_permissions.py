...
def test_404_for_not_purchased_materials(api, mock_notion_response, material_slug) -> None:
    api.get(f"/api/v2/notion/materials/{material_slug}/", expected_status_code=404)

    mock_notion_response.assert_not_called()


def test_ok_for_materials_with_same_page_id_from_another_course(api, mixer, another_course) -> None:
...
@pytest.mark.usefixtures("unpaid_order")
def test_ok_for_superuser(api) -> None:
...
@pytest.mark.usefixtures("unpaid_order")
def test_ok_for_user_with_permissions(api) -> None:
...
@pytest.mark.usefixtures("unpaid_order")
def test_superusers_do_not_fail_when_two_materials_with_the_same_id_are_present(api, mixer) -> None:
    api.user.add_perm("notion.material.see_all_materials")
