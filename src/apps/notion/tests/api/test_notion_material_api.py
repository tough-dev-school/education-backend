...
def test_both_formats_work_with_id(api, material_id, mock_notion_response) -> None:
...
def test_both_formats_work_with_slug(api, material_slug, mock_notion_response) -> None:
...
def test_content_is_passed_from_notion_client(api, material) -> None:
...
def test_page_block_goes_first_during_upstream_api_call(api, material) -> None:
...
def test_page_block_goes_first_for_cached_material(get_cached_material, material) -> None:
    got = get_cached_material(material.page_id)

    assert list(got.keys())[0] == "block-3"  # page block goes first


def test_extra_tags_are_dropped_during_upstram_api_call(api, material) -> None:
    got = api.get(f"/api/v2/notion/materials/{material.page_id}/")

    assert "_key_to_drop" not in got["block-1"]["value"]


def test_extra_tags_are_dropped_from_cached_material(get_cached_material, material) -> None:
    got = get_cached_material(material.page_id)

    assert "_key_to_drop" not in got["block-1"]["value"]


def test_non_fetched_assets_are_rewritten_to_notion_so_urls_during_upstream_api_call(api, material) -> None:
    got = api.get(f"/api/v2/notion/materials/{material.page_id}/")
...
def test_fetched_assets_paths_are_rewritten_during_upstream_api_call(api, material) -> None:
    got = api.get(f"/api/v2/notion/materials/{material.page_id}/")

    assert got["block-3"]["value"]["format"]["page_cover"] == "https://cdn.tough-dev.school/assets/typicalmacuser-downloaded.jpg"


def test_non_fetched_asset_paths_are_rewritten_to_notion_so_urls_for_cached_material(get_cached_material, material) -> None:
    got = get_cached_material(material.page_id)
...
@pytest.mark.usefixtures("fetched_asset")
def test_fetched_asset_paths_are_rewritten_for_cached_material(get_cached_material, material) -> None:
    got = get_cached_material(material.page_id)

    assert got["block-3"]["value"]["format"]["page_cover"] == "https://cdn.tough-dev.school/assets/typicalmacuser-downloaded.jpg"


def test_404_for_non_existant_materials(api, mock_notion_response) -> None:
    api.get("/api/v2/notion/materials/nonexistant/", expected_status_code=404)

    mock_notion_response.assert_not_called()


def test_404_for_inactive_materials(api, mock_notion_response, material) -> None:
