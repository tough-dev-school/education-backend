...
def test_call_with_lead(order_pusher, not_paid_order_with_lead, mock_push_new_order_to_amocrm, mock_push_existing_order_to_amocrm):
    order_pusher(order=not_paid_order_with_lead)

    mock_push_new_order_to_amocrm.assert_not_called()
    mock_push_existing_order_to_amocrm.assert_called_once_with(order_id=not_paid_order_with_lead.id)


def test_call_without_lead(order_pusher, not_paid_order_without_lead, mock_push_new_order_to_amocrm, mock_push_existing_order_to_amocrm):
    order_pusher(order=not_paid_order_without_lead)

    mock_push_existing_order_to_amocrm.assert_not_called()
    mock_push_new_order_to_amocrm.assert_called_once_with(order_id=not_paid_order_without_lead.id)


def test_call_paid(order_pusher, paid_order, mock_push_new_order_to_amocrm, mock_push_existing_order_to_amocrm):
    order_pusher(order=paid_order)

    mock_push_existing_order_to_amocrm.assert_not_called()
    mock_push_new_order_to_amocrm.assert_called_once_with(order_id=paid_order.id)


def test_linked_to_duplicate_order_lead_calls_update(
    order_pusher, not_paid_order_without_lead, not_paid_order_with_lead, mock_push_new_order_to_amocrm, mock_push_existing_order_to_amocrm
):
    order_pusher(order=not_paid_order_without_lead)
    ...
    mock_push_existing_order_to_amocrm.assert_called_once_with(order_id=not_paid_order_without_lead.id)
    mock_push_new_order_to_amocrm.assert_not_called()
...
def test_not_push_if_there_is_already_paid_order(
    order_pusher, not_paid_order_without_lead, paid_order, mock_push_new_order_to_amocrm, mock_push_existing_order_to_amocrm
):
    order_pusher(order=not_paid_order_without_lead)
...
def test_fails_if_many_lead_for_same_course_and_user(order_pusher, factory, not_paid_order_without_lead):
    ...
    with pytest.raises(AmoCRMOrderPusherException, match="There are duplicates for such order with same course and user"):
