...
def test_create_if_no_lead_not_paid(not_paid_order_without_lead, mock_create_lead):
    ...
    mock_create_lead.assert_called_once_with(order=not_paid_order_without_lead)


def test_update_if_with_lead_paid(paid_order_with_lead, mock_create_transaction, mock_update_lead):
    """Поступила оплата заказа - пушим весь заказ (обновляем Сделку и создаем Покупку)"""
    AmoCRMOrderPusher(order=paid_order_with_lead)()

    mock_create_transaction.assert_called_once_with(order=paid_order_with_lead)
    mock_update_lead.assert_called_once_with(status="purchased")


@pytest.mark.usefixtures("not_paid_order_with_lead")
def test_update_not_paid_order_linked_to_existing_lead(not_paid_order_without_lead, mock_update_lead, amocrm_lead):
...
@pytest.mark.usefixtures("not_paid_order_with_lead")
def test_update_paid_order_linked_to_existing_lead(paid_order_without_lead, amocrm_lead, mock_create_transaction, mock_update_lead):
    ...
    mock_create_transaction.assert_called_once_with(order=paid_order_without_lead)
    mock_update_lead.assert_called_once_with(status="purchased")


@pytest.mark.usefixtures("returned_order_with_lead")
def test_update_linked_to_existing_lead_from_returned_order_(paid_order_without_lead, amocrm_lead, mock_create_transaction, mock_update_lead):
    """
