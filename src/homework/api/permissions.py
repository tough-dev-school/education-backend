...
def get_all_purchased_user_ids(question: Question) -> frozenset[int]:
...
class ShouldHavePurchasedCoursePermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj) -> bool:
        return request.user.has_perm('homework.see_all_questions') or settings.DISABLE_HOMEWORK_PERMISSIONS_CHECKING or (request.user.id in get_all_purchased_user_ids(obj))


class ShouldHavePurchasedQuestionCoursePermission(permissions.BasePermission):
    def has_object_permission(self, request, view, obj) -> bool:
        return request.user.has_perm('homework.see_all_questions') or settings.DISABLE_HOMEWORK_PERMISSIONS_CHECKING or (request.user.id in get_all_purchased_user_ids(obj.question))
